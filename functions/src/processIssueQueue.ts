...
import { buildBundledBody, buildBundledTitle, FlattenedItem } from './issueQueue.js';
 ...
 * issue number + URL so the client can link to it afterwards.
 *
...
export async function processIssueQueue(
  ...
  for (const doc of snap.docs) {
    ...
    const items: FlattenedItem[] = rawItems.map((it: any) => ({
      ...
      submittedAtMs:
        it?.submittedAt?.toMillis?.() ?? Date.now(),
    }));
    ...
    try {
      ...
      // Back off: push dispatchAt out 5 minutes so we retry later rather than
      // hammering GitHub on every scheduler tick.
...
/**
 * Real GitHub poster used by the scheduler entrypoint. Separated so tests
 * can inject a fake without touching fetch.
 */
