...
- **services/** — Business logic & API calls: `*Service` classes (e.g., `ItemsService`, `HouseholdService`, `RecipeImportService`). Thin wrappers around Firestore & Cloud Functions. Utilities like `FuzzyMatch`, `TextItemParser`, `UnitConverter`, `CategoryGuesser`.
