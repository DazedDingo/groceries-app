...
- **widgets/** — Reusable UI components; organized by screen (e.g., `shopping_list/widgets/item_tile.dart`). `widgets/expandable_fab.dart` — generic single-FAB-with-fanned-children widget; both shopping list and pantry use it to consolidate every add path (manual, voice, bulk voice, bulk paste, scan barcode) under one primary "+", with a backdrop scrim that closes on tap.
...
- `go_router` routes defined in `lib/app.dart`; nested routes via `GoRoute` with path params. The five bottom-nav tabs are wrapped in a `StatefulShellRoute.indexedStack` so each tab keeps its own Navigator + per-screen state alive across switches — that's why `_restockChecked`, `_expiryChecked`, `_promotedThisSession`, `_sawNonEmptyList` and scroll positions persist when the user toggles tabs. Tapping the active tab pops to its root (`initialLocation: true`).
